...
def get_tds_invoices(tax_withholding_category, from_date, to_date, name, filter_existing=False, cond='', party_type=None, cost_center=None):
	...
	# Journal Entry (via Tax Withholding Entry) -- uses its own condition,
	# since `Tax Withholding Entry` only has a `tax_account` column (no `account`,
	# no `apply_tds`). Reusing `accounts_cond` here caused
	# "Unknown column 't1.account' in 'WHERE'".
	if len(accounts) == 1:
		je_accounts_cond = 'and t1.tax_account = "{}"'.format(accounts[0])
	else:
		je_accounts_cond = 'and t1.tax_account in ({})'.format('"' + '","'.join(accounts) + '"')
	...
	je_entries = frappe.db.sql("""select t.posting_date, t.name as invoice_no, 'Journal Entry' as invoice_type,
        ...
        CASE
            WHEN t1.withholding_amount > 0
            THEN t1.withholding_amount
            ELSE 0
        END AS tds_amount,
        COALESCE(t1.tax_account, '') AS tax_account,
        tre.tds_remittance,
        tre.tds_receipt_update,
        t.bill_no,
        t.bill_date,
		...
		from `tabJournal Entry` as t
			inner join `tabTax Withholding Entry` t1 on t.name = t1.parent
		...
		{existing_cond}
		{party_cond}
		{cond}""".format(je_accounts_cond=je_accounts_cond, cond=cond, existing_cond=existing_cond, \
			party_cond=party_cond, from_date=from_date, to_date=to_date, cost_center=cost_center), as_dict=True)
...
def get_tds_receipt_update(tds_remittance):
	res = frappe.db.sql("""
	    select parent as tds_receipt_update
